  ...
  return (
    <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div className="space-y-2">
        {kicker ? <div className="text-xs text-neutral-600">{kicker}</div> : null}
...
function Gallery({ images = [], label = "" }: { images: string[]; label?: string }) {
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {images.map((src, i) => (
        <div key={`${label}-${i}`} className="overflow-hidden rounded-3xl border bg-white">
          <img src={src} alt={`${label} photo ${i + 1}`} className="h-52 w-full object-cover" loading="lazy" />
        </div>
      ))}
    </div>
  );
...
function BeforeAfter({ beforeSrc, afterSrc, label = "" }: { beforeSrc: string; afterSrc: string; label?: string }) {
  const [mode, setMode] = useState<"before" | "after">("after");
  const src = mode === "before" ? beforeSrc : afterSrc;
  return (
    <div className="rounded-3xl border overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b bg-white/80 px-4 py-3">
        <div className="text-sm font-medium">Before / After</div>
        <div className="flex gap-2">
          <Button type="button" variant={mode === "before" ? "default" : "outline"} className="rounded-2xl" onClick={() => setMode("before")}>
            Before
          </Button>
          <Button type="button" variant={mode === "after" ? "default" : "outline"} className="rounded-2xl" onClick={() => setMode("after")}>
            After
          </Button>
        </div>
      </div>
...
export default function Page() {
  const CONTENT = useMemo(
    () => ({
      ...
      // ✅ Control what shows on the homepage (to keep it clean)
      featuredServiceIds: [
        "framing",
        "foundations",
        "siding",
      ...
      // ✅ Add as many services as you want here (front page won't clutter)
      services: [
        {
          id: "framing",
  ...
  return (
    <div className="min-h-screen bg-white text-black">
      {/* Top bar */}
      <div className="border-b bg-white/85 backdrop-blur sticky top-0 z-50">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-2xl border border-black/15 overflow-hidden bg-white">
              <img src={CONTENT.brand.logo} alt="Lifestyle logo" className="h-10 w-10 object-cover" />
            </div>
            <div>
              <div className="text-sm font-semibold leading-tight">{CONTENT.name}</div>
          ...
          <div className="flex md:hidden items-center gap-2">
            <Button size="sm" variant="outline" className="rounded-2xl" onClick={() => setOpenAllServices(true)}>
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 md:py-14">
          <div className="grid gap-10 md:grid-cols-2">
            <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.35 }} className="relative">
              <motion.p variants={fadeUp} custom={0} className="inline-flex items-center gap-2 rounded-full border border-black/15 px-3 py-1 text-xs text-neutral-700">
              ...
              <motion.h1 variants={fadeUp} custom={1} className="mt-5 text-4xl font-semibold tracking-tight md:text-5xl">
                ...
                <span className="block text-neutral-600">Framing, foundations, siding, and custom spaces.</span>
              </motion.h1>

              <motion.p variants={fadeUp} custom={2} className="mt-5 max-w-prose text-base leading-relaxed text-neutral-600">
              ...
              <motion.div variants={fadeUp} custom={4} className="mt-8 flex flex-wrap gap-3 text-sm text-neutral-700">
                {["Free estimates", "Licensed trades", "Residential + commercial"].map((t) => (
                  <span key={t} className="inline-flex items-center gap-2 rounded-2xl border border-black/15 px-3 py-2">
              ...
              <motion.div variants={fadeUp} custom={5} className="mt-8 flex flex-wrap gap-2">
                {featuredServices.slice(0, 5).map((s: any) => (
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-2xl border border-black/15 px-3 py-2 text-sm text-neutral-700 hover:bg-black/5 transition"
                  >
            ...
            <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.35 }} className="relative">
              <motion.div variants={fadeUp} custom={1} className="rounded-3xl border border-black/15 overflow-hidden">
                <img src={CONTENT.brand.heroCollage} alt="Lifestyle Homes & Services collage" className="h-[360px] w-full object-cover md:h-[420px]" />
              </motion.div>
                ...
                <Card className="rounded-3xl shadow-sm">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <p className="text-xs text-neutral-600">
                      Easy updates: add services inside <span className="font-medium">CONTENT.services</span> and choose homepage ones in <span className="font-medium">featuredServiceIds</span>.
                    </p>
                  </CardContent>
                </Card>
              </motion.div>
        ...
        <div className="mx-auto max-w-6xl px-4 py-14">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.25 }}>
            ...
            <div className="mt-8 space-y-10">
              {(() => {
                // Group featured services by category (order matters)
                        ...
                        return (
                          <motion.div key={s.id} variants={fadeUp} custom={i}>
                            <button type="button" onClick={() => setOpenService(s)} className="block w-full text-left">
                              <Card className="h-full rounded-3xl hover:shadow-sm transition">
                                <CardHeader>
                                  <div className="flex items-start gap-3">
                                    <div className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl border border-black/15">
                                    ...
                                    <div>
                                      ...
                                      <div className="mt-3 flex flex-wrap gap-2">
                                        {(s.tags || []).map((t: string) => (
                                          <Badge key={t} className="rounded-2xl">
                                            {t}
                                          </Badge>
                                        ))}
                                      </div>
                                ...
                                <CardContent>
                                  ...
                                  <div className="mt-4 inline-flex items-center text-sm">
                                    <span className="text-neutral-600">View photos & details</span>
            ...
            <div className="mt-10 rounded-3xl border border-black/15 p-4 text-sm text-neutral-700 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
              ...
              <Button className="rounded-2xl" onClick={() => setOpenAllServices(true)}>
            ...
            {CONTENT.projects.map((p: any, i: number) => (
              <motion.div key={p.id} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.25 }} variants={fadeUp} custom={i}>
                <button type="button" onClick={() => setOpenProject(p)} className="block w-full text-left">
                  <Card className="h-full overflow-hidden rounded-3xl hover:shadow-sm transition">
                    ...
                    <CardContent>
                      <div className="flex flex-wrap gap-2">
                        {p.tags.map((t: string) => (
                          <Badge key={t} className="rounded-2xl">{t}</Badge>
                        ))}
                      </div>
                      <p className="mt-3 text-sm text-neutral-600">{p.summary}</p>
                      <div className="mt-4 inline-flex items-center text-sm">
                        <span className="text-neutral-600">Open gallery</span>
                        <ArrowRight className="ml-2 h-4 w-4" />
                      </div>
                    </CardContent>
                  </Card>
        ...
        <div className="mx-auto max-w-6xl px-4 py-14">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {CONTENT.testimonials.map((t: any) => (
              <Card key={t.id} className="h-full rounded-3xl">
                <CardHeader>
                  <div className="flex items-start gap-3">
                    <div className="grid h-10 w-10 place-items-center rounded-2xl border border-black/15">
          ...
          <div className="grid gap-10 md:grid-cols-2">
            <div className="space-y-4">
              ...
              <Card className="rounded-3xl overflow-hidden">
                ...
                <CardContent>
                  <div className="overflow-hidden rounded-3xl border border-black/15">
      ...
      <footer className="border-t">
        <div className="mx-auto max-w-6xl px-4 py-10">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-3">
              <div className="grid h-10 w-10 place-items-center rounded-2xl border border-black/15 overflow-hidden">
                <img src={CONTENT.brand.logo} alt="Logo" className="h-10 w-10 object-cover" />
              </div>
              <div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button asChild className="rounded-2xl"><a href={`tel:${CONTENT.contact.phoneTel}`}><Phone className="h-4 w-4" /> Call</a></Button>
            </div>
          </div>

          <div className="mt-8 text-xs text-neutral-600">© {new Date().getFullYear()} {CONTENT.name}. All rights reserved.</div>
        </div>
      </footer>

      {/* ALL SERVICES MODAL (search + categories) */}
              ...
              <DialogHeader className="px-6 py-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Button className="rounded-2xl" onClick={goToContact}>Request Quote</Button>
                      ...
                      {list.map((s: any) => {
                        ...
                        return (
                          <button
                            ...
                            className="text-left"
                          >
                            <Card className="rounded-3xl hover:shadow-sm transition">
                              <CardHeader>
                                <div className="flex items-start gap-3">
                                  <div className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl border border-black/15">
                                    <Icon className="h-5 w-5" />
            ...
            <div className="max-h-[85vh] overflow-auto">
              <div className="sticky top-0 z-10 border-b bg-white/95 backdrop-blur">
                <DialogHeader className="px-6 py-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <DialogTitle className="text-xl">{openService.id === "saunas" ? "Sauna" : openService.title}</DialogTitle>
                    ...
                    <div className="flex items-center gap-2">
                      <Button className="rounded-2xl" onClick={goToContact}>Request Quote</Button>
                      <Button asChild variant="outline" className="rounded-2xl"><a href={`tel:${CONTENT.contact.phoneTel}`}><Phone className="h-4 w-4" /> Call</a></Button>
                      <Button variant="ghost" size="icon" className="rounded-2xl" onClick={() => setOpenService(null)}><X className="h-5 w-5" /></Button>
                    </div>
                  </div>
                </DialogHeader>
              </div>
          ...
          {openProject ? (
            <div className="max-h-[85vh] overflow-auto">
                ...
                <DialogHeader className="px-6 py-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <DialogDescription className="mt-1">{openProject.location} • {openProject.summary}</DialogDescription>
                      <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Button className="rounded-2xl" onClick={goToContact}>Request Quote</Button>
                      <Button asChild variant="outline" className="rounded-2xl"><a href={`tel:${CONTENT.contact.phoneTel}`}><Phone className="h-4 w-4" /> Call</a></Button>
                      <Button variant="ghost" size="icon" className="rounded-2xl" onClick={() => setOpenProject(null)}><X className="h-5 w-5" /></Button>
                    </div>
                  </div>
                </DialogHeader>
              </div>
              ...
              <div className="px-6 py-6 space-y-6">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button className="rounded-2xl" onClick={goToContact}>Request Quote</Button>
                  <Button asChild variant="outline" className="rounded-2xl"><a href={`mailto:${CONTENT.contact.email}`}>Email</a></Button>
                </div>
              </div>
            </div>
          ) : null}
